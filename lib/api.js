...
export function getAllMembers() {
	const membersDirectory = path.join(process.cwd(), '_members')
	const filenames = fs.readdirSync(memberssDirectory)

	return filenames.map(filename => {
		const file = fs.readFileSync(path.join(process.cwd(), '_members', filename), 'utf8')
